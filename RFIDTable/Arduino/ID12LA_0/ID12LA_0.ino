...
int ActualTable= 0;// Valid values are: 0 for the first table 1 for the second one
//int Actualtable= 1;
byte codes [2][12][5] = { 
                          {
...
int videoNumber = sizeof(codes[0])/ sizeof(codes[0][0]);

//#define DEBUG 

int sendVideo(byte *id){
  for(int i=0;i<videoNumber;i++){
  if(id[0] == codes[ActualTable][i][0] && id[1] == codes[ActualTable][i][1] && id[2] == codes[ActualTable][i][2] && id[3] == codes[ActualTable][i][3] && id[4] == codes[ActualTable][i][4] ){
      return i;
    }
  }
  return -1;
